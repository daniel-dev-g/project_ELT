...
def get_columns_file(file:str ,delimiter: str) -> str:
    ...
    columnas_str = ", ".join([f"[{col}] NVARCHAR(255)" for col in lf.collect_schema().names()])
    return columnas_str
...
def table_creator(execution_id:str, engine, schema:str, table:str, columns:str):
    ...
    if check_table_exists(engine, schema, table ):
        registrar_log("table_already_exists", {"execution_id":execution_id,"tabla": table, "schema": schema})
        return
