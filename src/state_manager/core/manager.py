...
class StateManager:
    def __init__(self, process_name: str = "ETL_Process"):
        ...
        self.active_files = {}  # Track archivos activos: {file_id: file_name}
    ...
    def start_file_processing(self, file_path: str, task_info: dict) -> int:
        ...
        with get_db_cursor() as cursor:
            cursor.execute(
                ...
                (self.execution_id, file_path, file_name, file_size, task_info['table_destination'], task_info['schema'])
            )
    ...
    def complete_process(self, status: str = 'COMPLETED') -> str:
        ...
        with get_db_cursor() as cursor:
            ...
            cursor.execute(
                ...
                (status, stats[0] if stats else 0, stats[1] if stats else 0, stats[2] if stats else 0, stats[3] if stats else 0, self.execution_id)
            )
